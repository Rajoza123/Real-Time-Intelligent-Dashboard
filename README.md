...
The **Ranked Alerts** panel on the dashboard displays a live feed of all alerts generated by the backend, classified and ranked in real time.  
Each column in the table provides key insight into the event and its computed importance:

| Column | Description |
|--------|--------------|
| **Camera** | The camera ID that generated the event (e.g., CAM-01, CAM-03). |
| **Label** | The detected object or condition — e.g., Fire, Smoke, Intrusion, Person, Crowd. |
| **Severity** | The alert classification: **Critical**, **Moderate**, or **Low** — derived using a weighted ranking formula. |
| **Confidence** | Model detection confidence (0–1 range). Higher values indicate stronger certainty. |
| **Freq (win)** | The number of times the same camera triggered alerts within the recent time window (for frequency analysis). |
| **Time** | The timestamp when the event was generated, in ISO format. |
| **Score** | The computed ranking score combining label weight, frequency, and confidence — used to determine severity. |

### 🧠 How It Works
- The backend continuously emits new events from multiple cameras.  
- Each event is evaluated using this formula:

Score = (Label Weight) + (0.4 × Confidence) + (0.35 × Frequency Term)
